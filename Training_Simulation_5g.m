...
str = sprintf('%s\\Myoware_500Hz_R_5g_6.csv', file_path); % File path to logged data
...
% 500 Hz
tempor = round((size(data,1)-62500));
data = data((tempor+1:end),:); % We should have (5*5*(5s)*500Hz) = 62,500 data points
...
for q = 1:5
    ...
    for n=1:num_test
        ...
        prediction(1,n) = LDA_Predict_v2(testing(q).trial(:,n), Wg, Cg);
    end
